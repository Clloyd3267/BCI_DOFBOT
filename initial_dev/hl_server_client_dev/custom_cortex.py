...
class Cortex(Dispatcher):
    ...
    def sub_request(self, stream):
        ...
        # Saves the session sid
        if self.first:
            self.sid = result_dic.get('sid')
            self.first = False

        # Waits for new profile to be loaded
        while result_dic.get("id") is None:
            new_data = self.ws.recv()
            result_dic = json.loads(new_data)

        # Changes to mental command mode
        if result_dic.get("id") != 6:
    ...
    def setup_profile(self, profile_name, status, newName = None):
